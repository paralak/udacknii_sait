...
- `___blocks/u_bot_datas.py` — статические словари (consumption_matrix, НЗ, расписания)
- `___blocks/u_bot_getorder.py` — запись заказа в БД

БД бота: `python_bot` на `10.182.226.152:3306` (тот же сервер, другая БД).

### Ключевые данные из u_bot_datas.py (мигрированы / используются)

**`rashodniki_postav`** — кто что поставляет по адресам (источник истины о supplier_role):
- **GDT** (`post_gdt`): стаканы/крышки/расходники (sku 2,4,6,7,8,9,10,11), сиропы (12-15) → поставляет **на A10 (РЦ)**, `via_rc=1`
- **АЛЕФ** (`post_alef`): кофе (sku 0) для A1/A2/A4, чай (16-21) для большинства → поставляет **на A10**, `via_rc=1`
- **КМС** (`post_kms`): кофе (sku 0) для A3,A5-A9 → **прямая доставка** в магазины по вторникам, `via_rc=0`
- **ИП Харисова** (`post_u_vatch`): молоко (sku 1) → прямая доставка пн/ср/пт
- **ПРОМИНДУСТРИЯ** (`post_prom`): стаканы 250/400мл (sku 3,5) → прямая доставка по четвергам
- Сиропы (12-15) и чай без явного поставщика → идут через A10 (`post_A10`)

**`postavshik_min_sum`** — минимальные суммы заказа (мигрированы в `supplier_settings.min_order_sum`):
- КМС: 18 000 ₽ для всех магазинов
- GDT: 7 500 ₽ для A1-A4, 6 000 ₽ для A5-A9
- АЛЕФ: 12 000 ₽ для A1-A4, 6 000 ₽ для A5-A9

**`consumption_matrix`** — ТТК (технико-технологические карты): расход ингредиентов на единицу напитка.
Используется для расчёта `sku_rashod` из данных продаж:
```python
# Пример: капучино 0.2
'ca02': {
    0: 0.012,   # Кофе (кг)
...
расход_ингредиента = Σ (продажи_напитка × consumption_matrix[напиток][ингредиент]) / дней
```
Матрица хранится в боте как статический словарь. При переносе расчёта в NestJS — вынести в БД
или константы модуля `consumption.service.ts`.
